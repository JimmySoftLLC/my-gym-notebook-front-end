...
const getBatch = async (myIds: any[]) => {
  ...
  for (let i = 0; i < myGymMembers.length; i++) {
    myGymMembers[i].firstName =
      myGymMembers[i].firstName === blankPlaceHolder
        ? ''
        : myGymMembers[i].firstName;
    myGymMembers[i].lastName =
      myGymMembers[i].lastName === blankPlaceHolder
        ? ''
        : myGymMembers[i].lastName;
    myGymMembers[i].email =
      myGymMembers[i].email === blankPlaceHolder ? '' : myGymMembers[i].email;
    myGymMembers[i].bio =
      myGymMembers[i].bio === blankPlaceHolder ? '' : myGymMembers[i].bio;
    myGymMembers[i].exerciseIdsJSON =
      myGymMembers[i].exerciseIdsJSON === undefined
        ? JSON.parse('[]')
        : JSON.parse(myGymMembers[i].exerciseIdsJSON);
    myGymMembers[i].teamMateIdsJSON =
      myGymMembers[i].teamMateIdsJSON === undefined
        ? JSON.parse('[]')
        : JSON.parse(myGymMembers[i].teamMateIdsJSON);
    myGymMembers[i].gymDayIdsJSON =
      myGymMembers[i].gymDayIdsJSON === undefined
        ? JSON.parse('[]')
        : JSON.parse(myGymMembers[i].gymDayIdsJSON);
    myGymMembers[i].workoutIdsJSON =
      myGymMembers[i].workoutIdsJSON === undefined
        ? JSON.parse('[]')
        : JSON.parse(myGymMembers[i].workoutIdsJSON);
    myGymMembers[i].dataJSON =
